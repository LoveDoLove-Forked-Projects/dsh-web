...
export const LIANGSHEN_GUIDANCE = '本机已安装 dsh-liangshen 插件（梁神模式 agent preset）：新建会话的预设选择器中可选「梁神模式」。原理：系统提示词永久保持官方 Minimal 那一行 persona（minimal-prompt 只放行该段与 plan 模式的 plan:policy，并在组装时追加一行工作区目录 Your working directory is <cwd>.）；工具清单（完整标准目录，名称加一行摘要）由 tool-catalog 从第一条用户消息起以 user 消息注入在用户消息之后，形如 skill catalog，仅当工具集变化或该消息离开可见面（压缩、恢复）时重发；wire 上的 schema 按锚定回合分层：首个回合只保留 bash、str_replace_editor、exit_plan_mode、skill 四个 schema，第二个回合起换成官方 Standard 的完整工具目录（工具执行按名字解析会话注册表，不依赖请求里是否声明），没有 PTC 切换、没有输出预算上限。工作区指令的全文注入被替换为一次性的引用文件提示（instructionHint）。preset 文件由插件维护于 ~/.dsh/.agent-presets，升级插件时自动更新；默认预设由用户自行选择。用户提到「梁神模式 / 锚定模式 / anchored standard」时即指本插件，请据此协作。'
