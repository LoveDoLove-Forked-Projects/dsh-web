...
| `anchorTools` | `[]` | Tool names that alone form the anchor turn's wire; the full roster engages from the second turn. Empty disables staging (full catalog from the first request). The shipped preset sets `bash`, `str_replace_editor`, `exit_plan_mode`, `skill`. |
