...
两个 Bug 报告保留开放并给出明确立场，而不是静默关闭：

- #1452：崩溃需要 DSH 宿主低于聚合包声明的最低版本（`>=0.1.5-rc.1`）。把 `dsh-better-sidebar` 退回 0.18.0 能保护低版本宿主，但会推翻当天的 [0.1.5-rc.1 挡位决定](../architecture/2026-09-10-sdk-cohort-0.1.5-rc.1.md)，且旧构建没有针对 rc.1 的冒烟，因此把选择留给 owner；报告者已拿到两条可用路径（升级 DSH，或把聚合包钉回 0.3.19）。
- #1397：根因是官方 `@deepseek-ai/dsh-tools` 内部用私有 `Symbol` 作注册键，0.1.5-rc.1 仍然如此；本仓库没有修复位置，继续跟踪上游。

当时开放的另外两个增强类 Issue（#1439、#1448）按范围外处理，未做任何改动。

## Alternatives considered

- 以"受支持宿主上无法复现"关闭 #1452。否决：解决了报告者这一次，却掩盖了契约执行缺口（宿主不读 `dsh.engines.dsh`，只有网关更新通道会拦截低版本更新），并替 owner 预支了一个仓库已经做过的产品决定。
...
- 开放 Bug 列表现在只剩一个产品决定项（#1452）和一个上游跟踪项（#1397），都不阻塞本仓库的后续工作。
