...
- The first request carries only the anchor schemas, but the model can still act on the whole catalog from the first turn: registry-name execution covers the not-yet-schematized tools, at the cost of guessed parameters until the full schemas arrive at the boundary.
