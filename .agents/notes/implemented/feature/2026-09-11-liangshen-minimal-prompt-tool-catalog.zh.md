...
preset 把官方 Minimal 的一行 persona——外加本模式的固定工作纪律与一行工作区目录方位信息——作为整个系统提示词，并从第一条用户消息起就把官方 Standard 的完整工具目录作为上下文注入送达，而它的 schema 从会话的第二个回合起才上 wire——锚定回合（首个回合）只带一小份 `anchorTools` 面，由同一个插件分层（[按锚定回合分层](2026-09-11-liangshen-anchor-turn-staging.zh.md)）。模型的能力事实以提示词尾部的消息送达，而不是以系统提示词散文承载。
