...
- The arm reports the session's `agentPreset` projection rather than local state, so a reload shows the truth, and it is only operable while the session is blank — the same window the host accepts. Outside that window it renders as locked with the reason.
...
- The lever is scoped to the new-session screen by state, not by slot: the slot renders for every session, and the component reports `locked` for a session that has started.
...
- `tests/lever-ui.spec.tsx` renders the component under jsdom: the semantic attributes, the `role="switch"` state, pointer pulls and pushes, keyboard activation, locked and missing states, the refusal line, and the burst appearing once per landed pull.
...
- The lever cannot enable a mode the deployment has not installed: with the preset absent it renders `missing` and says so, which is also the state a user sees when the plugin row is disabled.
